/**
 * Taken from 'https://github.com/adis300/fft-c' on 28-09-2020
 *
...
static void dradfg(int ido, int ip, int l1, int idl1, FFT_PRECISION *cc,
		FFT_PRECISION *c1,
		FFT_PRECISION *c2, FFT_PRECISION *ch, FFT_PRECISION *ch2,
		FFT_PRECISION *wa) {
...
FFTTransformer * create_fft_transformer(int signal_length, int scale_output) {
	FFTTransformer * transformer = (FFTTransformer *) malloc(
			sizeof(FFTTransformer));
	transformer->ifac = (int *) calloc(FFT_IFAC, sizeof(int));
	transformer->wsave = (FFT_PRECISION *) malloc(
			(2 * signal_length + 15) * sizeof(FFT_PRECISION));
...
FFTCosqTransformer * create_fft_cosq_transformer(int signal_length,
		int scale_output) {
	FFTCosqTransformer * transformer = (FFTCosqTransformer *) malloc(
			sizeof(FFTCosqTransformer));
	transformer->ifac = (int *) calloc(FFT_IFAC, sizeof(int));
	transformer->wsave = (FFT_PRECISION *) malloc(
			(3 * signal_length + 15) * sizeof(FFT_PRECISION));
	transformer->n = signal_length;
	if (scale_output == FFT_SCALED_OUTPUT)
		transformer->scale_output = FFT_SCALED_OUTPUT;
	else
